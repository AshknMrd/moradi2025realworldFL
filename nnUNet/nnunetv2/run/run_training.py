...
def maybe_load_checkpoint(nnunet_trainer: nnUNetTrainer, continue_training: bool, validation_only: bool,
    ...
    if continue_training:
        ...
        if pretrained_weights_file is not None:
            ...
            # Added to load the optimizer from the check point
            pretrained_checkpoint = torch.load(pretrained_weights_file, weights_only=False)
            if 'optimizer_state' in pretrained_checkpoint and hasattr(nnunet_trainer, 'optimizer'):
                pretrained_checkpoint['optimizer_state']['param_groups'][0]['initial_lr'] = pretrained_checkpoint['optimizer_state']['param_groups'][0]['lr']
                nnunet_trainer.optimizer.load_state_dict(pretrained_checkpoint['optimizer_state'])
                # nnunet_trainer.current_epoch = pretrained_checkpoint['current_epoch']
                print("\n Optimizer state and currewnt epoch loaded from pretrained checkpoint. \n")
...
def run_training(dataset_name_or_id: Union[str, int],
                 ...
                 current_round: Optional[int] = None, # me add
                 total_rounds: Optional[int] = None, # me add
                 device: torch.device = torch.device('cuda')):
    ...
    if num_gpus > 1:
        ...
        if total_rounds is not None: # me add
            setattr(nnunet_trainer, "current_round", current_round)
            setattr(nnunet_trainer, "total_rounds", total_rounds)
...
def run_training_entry():
    ...
    parser.add_argument("-current_round", type=int, required=False, default=None, help="The current FL round number.") # new add
    parser.add_argument("-total_rounds", type=int, required=False, default=None, help="The total FL round number.") # new add
    ...
    run_training(args.dataset_name_or_id, args.configuration, args.fold, args.tr, args.p, args.pretrained_weights,
                 ...
                 args.current_round, args.total_rounds, # me add
                 device=device)
