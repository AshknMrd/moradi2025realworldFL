...
def extract_fingerprint_dataset(dataset_id: int,
                                ...
                                num_processes: int = default_num_processes, dataset_json_dir: str = None, check_dataset_integrity: bool = False,
                                clean: bool = True, verbose: bool = True): # ADDED
    """
    ...
    if check_dataset_integrity:
            verify_dataset_integrity(join(nnUNet_raw, dataset_name), dataset_json_dir, num_processes)# ADDED

    fpe = fingerprint_extractor_class(dataset_id, num_processes, dataset_json_dir, verbose=verbose) # ADDED
...
def extract_fingerprints(dataset_ids: List[int], fingerprint_extractor_class_name: str = 'DatasetFingerprintExtractor',
    ...
    for d in dataset_ids:
        extract_fingerprint_dataset(d, fingerprint_extractor_class, num_processes, dataset_json_dir, check_dataset_integrity, clean,
                                    verbose) # ADDED


def plan_experiment_dataset(dataset_id: int,
                            dataset_json_dir: str = None,
...
def plan_experiments(dataset_ids: List[int], dataset_json_dir, experiment_planner_class_name: str = 'ExperimentPlanner',
                     gpu_memory_target_in_gb: float = None, preprocess_class_name: str = 'DefaultPreprocessor',
                     overwrite_target_spacing: Optional[Tuple[float, ...]] = None,
                     overwrite_plans_name: Optional[str] = None): #ADDED
    """
    overwrite_target_spacing ONLY applies to 3d_fullres and 3d_cascade fullres!
    """
    ...
    for d in dataset_ids:
        _, plans_identifier = plan_experiment_dataset(d, dataset_json_dir, experiment_planner, gpu_memory_target_in_gb,
                                                      preprocess_class_name,
                                                      overwrite_target_spacing, overwrite_plans_name) # ADDED
    return plans_identifier


def preprocess_dataset(dataset_id: int,
                       dataset_json_dir: str = None, #ADDED
    ...
    if dataset_json_dir is not None: #ADDED
        dataset_json = load_json(dataset_json_dir) #ADDED
    else: 
        dataset_json = load_json(join(nnUNet_raw, dataset_name, 'dataset.json')) #ADDED
...
def preprocess(dataset_ids: List[int],
               ...
               verbose: bool = False):
    for d in dataset_ids:
        preprocess_dataset(d, dataset_json_dir, plans_identifier, configurations, num_processes, verbose) #ADDED
