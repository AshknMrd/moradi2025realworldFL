...
def verify_dataset_integrity(folder: str, folder_dataset_json: str, num_processes: int = 8) -> None: #ADDED
    """
    ...
    :return:
    """
    if folder_dataset_json is not None: # ADDED
        assert isfile(folder_dataset_json), f"There needs to be a dataset.json file in folder, folder={folder}" # ADDED
        dataset_json = load_json(folder_dataset_json) # ADDED
    else:
        assert isfile(join(folder, "dataset.json")), f"There needs to be a dataset.json file in folder, folder={folder}" # ADDED
        dataset_json = load_json(join(folder, "dataset.json")) # ADDED
